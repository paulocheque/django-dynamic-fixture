...
class DataFormatTest(DDFTestCase):
    def test_it_must_generate_random_numbers(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat('###'))
        assert re.match(r'\d{3}', instance.string)

    def test_it_must_generate_lower_ascii_chars(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat('___'))
    ...
    def test_it_must_accept_pure_chars(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat('ABC123'))
        assert re.match(r'ABC123', instance.string)

    def test_it_must_be_able_to_escape_symbols(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat(r'!# !_ !-'))
        assert '# _ -' == instance.string

    def test_phone_mask(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat(r'+## (##) #####!-#####'))
        assert re.match(r'\+\d{2} \(\d{2}\) \d{5}-\d{5}', instance.string)

    def test_address_mask(self):
        instance = self.ddf.get(ModelWithStrings, string=DataFormat(r'St. -______, ### !- -- --'))
